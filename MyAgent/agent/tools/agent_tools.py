...
def _get_public_ip() -> str:
    # 可在agent.yml里覆盖
    # agent_conf是从config / agent.yml读取的配置字典。agent_conf.get("public_ip_sources", 默认值)
    # 意思是：- 如果配置里写了 public_ip_sources，就使用配置值；- 没写就使用默认公网IP服务。
...
def get_weather(city: str) -> str:
    ...
    try:
        ...
        #如果写 `humidity = live.get("humidity", "未知")` → 找不到就赋值`"未知"`，**程序正常继续跑**
